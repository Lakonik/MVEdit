...
from ...core import eval_psnr, eval_ssim_skimage, reduce_mean, rgetattr, rsetattr, extract_geometry, \
    module_requires_grad, get_cam_rays, get_rays, get_ray_directions, depth_to_normal
...
class BaseNeRF(nn.Module):
    ...
    def ray_sample(self, cond_rays_o, cond_rays_d, cond_imgs, n_samples, sample_inds=None, cond_extras=None):
        """
        Args:
            ...
            cond_extra (None | list[torch.Tensor]): tensors with shape (num_scenes, num_imgs, h, w, n_extra_dims)
        ...
        if self.patch_loss is None:
            ...
            target_extras = [] if cond_extras is None else [cond_extra.reshape(
                num_scenes, num_scene_pixels, cond_extra.size(-1)) for cond_extra in cond_extras]
        else:
            ...
            target_extras = [] if cond_extras is None else [cond_extra.reshape(
                    num_scenes, -1, h // self.patch_size, self.patch_size, w // self.patch_size, self.patch_size, cond_extra.size(-1)
                ).permute(0, 1, 2, 4, 3, 5, 6).reshape(num_scenes, -1, self.patch_size, self.patch_size, cond_extra.size(-1))
                for cond_extra in cond_extras]
        if num_scene_pixels > n_samples:
            if sample_inds is None:
                sample_inds = [
                    torch.randperm(
                        target_rgbs.size(1), device=device
                    )[:n_samples if self.patch_loss is None else n_samples // (self.patch_size ** 2)]
            ...
            target_extras = [target_extra[scene_arange, sample_inds] for target_extra in target_extras]
        if self.patch_loss is not None:
            rays_o = rays_o.reshape(num_scenes, -1, 3)
            rays_d = rays_d.reshape(num_scenes, -1, 3)
            target_rgbs = target_rgbs.reshape(-1, self.patch_size, self.patch_size, 3)
            target_extras = [target_extra.reshape(
                -1, self.patch_size, self.patch_size, target_extra.size(-1)) for target_extra in target_extras]
        return rays_o, rays_d, target_rgbs, *target_extras
        ...
        with module_requires_grad(decoder, False):
            ...
            for inverse_step_id in range(n_inverse_steps):
                ...
                out_rgbs, loss, loss_dict = self.loss(
                    ...
                    target_rgbs, rays_o, rays_d, dt_gamma, scale_num_ray=num_scene_pixels, cfg=cfg,
    ...
    def render(self, decoder, code, density_bitfield,
               h, w, intrinsics, poses, cfg=dict(), bg_color=None, perturb=False,
               normal_bg=[0.5, 0.5, 1.0]):
        if bg_color is None:
        ...
        for rays_o_single, rays_d_single in zip(rays_o, rays_d):
            ...
            if not return_rgba:
                rgbs = (torch.stack(outputs['image'], dim=0) if num_scenes > 1 else outputs['image'][0]) \
                    + bg_color * (1 - weights_sum.unsqueeze(-1))
        ...
        inverse_z_depth = cfg.get('inverse_z_depth', True)
        if inverse_z_depth:
            out_depth = out_depth * torch.linalg.norm(directions, dim=-1)  # from 1/r to 1/z

        decoder.train(decoder_training_prev)
        if cfg.get('compute_normal', False):
            assert inverse_z_depth and return_rgba
            out_depth_fg = out_depth / out_image[..., 3].clamp(min=1e-6)
            out_normal_fg = depth_to_normal(out_depth_fg, directions)
            out_normal = out_normal_fg * out_image[..., 3:] + out_normal_fg.new_tensor(normal_bg) * (1 - out_image[..., 3:])
            return out_image, out_depth, out_normal, out_normal_fg
        else:
